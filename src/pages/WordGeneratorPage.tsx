...
const PROMPT_EXAMPLES: { label: string; text: string }[] = [
  { label: "Проект про робототехнику", text: "Сделай мне школьный проект docx про робототехнику" },
  { label: "Реферат про космос", text: "Напиши реферат про историю освоения космоса" },
  { label: "Сочинение о дружбе", text: "Составь сочинение на тему «Что значит быть настоящим другом»" },
];
...
export const WordGeneratorPage: React.FC<WordGeneratorPageProps> = ({
  ...
  const isPromptMode = rawText.trim().length >= 8 && isWordGenerationPrompt(rawText);
  const minInputLength = minWordInputLength(rawText);
  ...
  const runGenerate = async (sourceText?: string) => {
    const trimmed = (sourceText ?? rawText).trim();
    const requiredLength = minWordInputLength(trimmed);
    if (!trimmed || trimmed.length < requiredLength) {
      setError(
        isWordGenerationPrompt(trimmed)
    ...
    const styleToUse = category === "school" ? inferSchoolDocStyle(trimmed) : docStyle;
    if (category === "school") setDocStyle(styleToUse);

    const mode = resolveWordGenerationMode(trimmed);
    const titleToUse =
      docTitle.trim() ||
      (mode === "prompt" ? extractWordPromptTopic(trimmed) : undefined);
    ...
    try {
      ...
      if (mode === "prompt" && !docTitle.trim() && data.document?.title) {
        setDocTitle(String(data.document.title));
      }
      setResult(data.document as WordDocumentData);
    } catch (err: any) {
      setError(err.message || "Ошибка генерации. Попробуйте ещё раз.");
      const local = buildLocalWordDocument(trimmed, styleToUse);
  ...
  useEffect(() => {
    ...
    setCategory("school");
    const topic = extractWordPromptTopic(initialText);
    setDocTitle(topic);
    setDocStyle(inferSchoolDocStyle(initialText));

    if (autoGenerate && authToken && !autoGenerateDone.current) {
      autoGenerateDone.current = true;
      void runGenerate(initialText);
    }
  }, [initialText, autoGenerate, authToken]);
  ...
  const applyPromptExample = (example: (typeof PROMPT_EXAMPLES)[number]) => {
    ...
    setDocTitle(extractWordPromptTopic(example.text));
    setDocStyle(inferSchoolDocStyle(example.text));
    setFontId(getDefaultFontForStyle(inferSchoolDocStyle(example.text)));
    setResult(null);
  };
        ...
        <div className="space-y-4">
          <div className="rounded-2xl border border-white/10 bg-[#111114] p-4 sm:p-5">
              ...
              <div className="flex flex-wrap gap-2 mb-4">
                {PROMPT_EXAMPLES.map((ex) => (
                  <button
                    ...
                    className="text-[10px] rounded-full border border-violet-500/30 bg-violet-500/15 text-violet-200 px-3 py-1 hover:bg-violet-500/25 transition-colors cursor-pointer"
                  >
                    {ex.label}
                  </button>
                ))}
            ...
            <label className="text-[10px] font-mono uppercase tracking-widest text-slate-500 mb-2 block">
              {isPromptMode ? "Запрос или черновик" : "Исходный текст"}
            </label>
            <textarea
              value={rawText}
              ...
              placeholder={
                category === "school"
                  ? "Запрос: «Сделай проект docx про робототехнику» — или вставьте черновик с тезисами..."
            ...
            <div className="flex items-center justify-between mt-2 text-[10px] text-slate-600 font-mono">
              ...
              <span>
                {isPromptMode ? "режим: запрос · мин. 12" : `мин. ${minInputLength}`}
              </span>
            </div>
            {isPromptMode && (
              <p className="mt-2 text-[11px] text-violet-300/90 leading-relaxed">
                ИИ напишет документ по теме «{extractWordPromptTopic(rawText)}» и оформит его в Word.
              </p>
            )}
          </div>
